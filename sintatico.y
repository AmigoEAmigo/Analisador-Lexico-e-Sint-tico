%{
    ...
    %%
    //A gramática é definida usando regras do Yacc/Bison:
    S: programa { printf("SUCCESSFUL COMPILATION."); return;}
    ;//Se o programa estiver correto, será exibida a mensagem SUCCESSFUL COMPILATION..
        
    programa: declaracoes prog1
        | funcao prog1
    ;//Define que um programa pode consistir em declarações ou funções.
    ...
    %%
    //Se um erro sintático for encontrado, a função yyerror é chamada:
    yyerror(char *s)
    {   
        if(ajuda_sint == 0){
    ...
    //A função main executa o analisador sintático e léxico:
    ...
    //printf("%s     %s\n\n\n", vets2,str);
        ...
        str34[indice] = '\0';
        //Se houver um erro, exibe a linha e a coluna onde ocorreu:
        if(ajuda_sint == 1){
